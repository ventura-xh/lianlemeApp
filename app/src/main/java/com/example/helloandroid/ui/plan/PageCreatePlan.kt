...
import com.example.helloandroid.viewmodel.CreatePlanViewModel
...
fun PageCreatePlan(
    ...
    viewModel: CreatePlanViewModel = viewModel(factory = CreatePlanViewModel.factory)
) {
    ...
    // ✅ 监听从选择动作页面返回的数据
    LaunchedEffect(Unit) {
        navController.currentBackStackEntry?.savedStateHandle?.getLiveData<String>("selected_action_ids")
            ?.observeForever { idsString ->
                idsString?.let {
                    ...
                    navController.currentBackStackEntry?.savedStateHandle?.remove<String>("selected_action_ids")
                }
            }
    }
            ...
            TopAppBar(
                ...
                actions = {
                    Button(
                        ...
                        enabled = selectedActions.isNotEmpty() && !isSaving
                    ) {
                        Text("保存")
                    }
                },
        ...
        Column(
            ...
            if (selectedActions.isEmpty()) {
                ...
                LazyColumn(
                    ...
                    itemsIndexed(selectedActions) { index, planAction ->
